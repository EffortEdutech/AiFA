...
## Volume 12_1 — Series 12: Web Platform Architecture — Version 1.4 (Proposed)
...
**Version 1.4 change note:** Series 13's Sprint 22 key-wrapping design review (`docs/sprint-plan/Phase_3_Accounting_Compliance_Operations/Sprint_22_Multi_User_Key_Wrapping_Design_Review.md`, 2 September 2026) found that ADR-003's single-active-device write lock — designed and correct for one owner's own multiple devices — becomes unworkable if applied unmodified once a business has more than one *person* on it: it would mean only one device across an entire team could ever write at a time. The owner reviewed this finding and directed that it be resolved in direction now, before Sprint 23 designs the `active_device_lock`/`devices` schema migration, rather than deferred to a later sprint. Section 5b (new) records the resolved direction: the lock's scope moves from per-business to per-`BusinessMembership`. Section 11 and Section 12 are updated to reflect this as a required input to Sprint 23, not an open item.
...
## 5b. Team-Mode Amendment: Per-Membership Write-Lock Scope (new in Version 1.4)

Sections 5a-6a above describe and remain correct for **single-owner, multi-device** use: one person, several of their own devices, trading write access among themselves. Series 13 (Vol 13_1) introduces `BusinessMembership` — more than one *person* authorised against the same business. Applying `public.active_device_lock` unmodified in that world — one lock row per business, full stop — would mean an Owner, a Bookkeeper, a Sales Agent, and a Payroll Admin could never write concurrently: whichever one of them last took the lock blocks everyone else, business-wide, until they explicitly hand it off. That is not a rough edge; it makes the entire point of Series 13 — genuine concurrent team use — impossible as designed. This was surfaced during Sprint 22's key-wrapping review (which needed to state plainly how key rotation interacts with the write lock) and is recorded here rather than left as an implicit gap between Series 12 and Series 13.

**Resolved direction:** the active-device lock's scope moves from **per-business** to **per-`BusinessMembership`**. Concretely:

- `public.active_device_lock` gains a `business_membership_id` column and its uniqueness/atomicity guarantee (Section 5a.2) is re-scoped to "exactly one active device per membership," not "exactly one active device per business." A solo business (exactly one membership, the Owner) sees zero behavioural change — this degenerates back to today's per-business lock exactly, satisfying Vol 13_3's solo-mode-is-free requirement by construction, the same way Sprint 24's `effective_access_model` is required to.
- Two different members' devices can hold their own active lock and write concurrently. Safety across members no longer comes from mutual exclusion (there isn't any, by design) — it comes from what was already true underneath: `sync_envelopes` is append-only and ordered by `server_seq`, and RLS (redesigned in Sprint 23, Vol 13_1 §10) governs what each membership's rows may even be inserted against. Two members writing at the same moment is exactly as safe as two members writing a second apart; there is no shared mutable row between them for the lock to protect, unlike within one member's own multi-device set.
- Primary-device designation (Section 5a.4) and the takeover protocol (Section 6a) are unchanged in shape, just re-scoped the same way: each membership has its own primary device among its own devices, and a primary takeover only ever contests that membership's own other devices — never another member's.
- `devices` gains the same `business_membership_id` scoping (a device belongs to exactly one membership, matching Vol 13_1 §4's model of a person's own devices).

**What this sprint does not resolve:** the exact RPC/migration shape (`register_device`, `request_activation`, `request_primary_takeover`, `set_primary_device`, `revoke_device` all need their `business_id`-scoped queries reworked to `business_membership_id`-scoped ones) is implementation, not architecture, and belongs to whichever sprint actually migrates the schema. Given the owner's 2 September 2026 direction that this be resolved *before* schema is written rather than patched after, **Sprint 23's own RLS/schema migration task (which already touches `devices`/`active_device_lock`, per that sprint's Task Breakdown) is the sprint that implements this re-scoping**, not a separate later sprint — see that sprint's plan for the concrete task.
...
- Vol 4_0_0 — ADR-002 (cloud sync hub shape), ADR-003 (single active-device write lock — amended in Version 1.4, Section 5b, to scope per-`BusinessMembership` rather than per-business once Series 13 team mode exists), and ADR-004 (primary device designation and forced takeover), the decision records this volume is the detailed design for.

## 12. Open Items (stated honestly, not hidden)

- **Per-membership write-lock scope (Section 5b) is resolved in direction, not yet implemented.** The concrete RPC/migration rework is Sprint 23's task, not this volume's — this is recorded here so Section 12 stays the honest single place tracking what's designed vs. built, per this volume's own stated purpose for this section.
