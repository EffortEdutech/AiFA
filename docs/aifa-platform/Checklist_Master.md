...
**Status: PARTIAL — backend verified live; browser rendering NOT yet confirmed (correction below).** A genuine architecture correction, raised by the owner after testing `localhost:3070` and finding none of the new Public Site work visible there.
...
**Not yet verified (owner action required):**
- [ ] Set the Vercel project's **Root Directory** to `web-public` (Project Settings -> General) — without this, Vercel will try to build the repo root and fail, since there's no `package.json` there matching a framework.
- [ ] Set `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY` as Vercel Environment Variables (Production + Preview) — values and where to find them are in `web-public/README.md`. Anon/publishable key only, confirmed safe to expose (browser-facing by design), not the service_role key.
- [ ] Push this commit to the branch Vercel deploys from, then visit `https://ai-fa-tawny.vercel.app/site/verify-test-co` in a real browser to confirm it renders as an actual styled page (not raw text, not a 404) — this is the real end-to-end test the raw-text bug above was blocking.
- [ ] Once confirmed, re-test the contact form end-to-end (submits via `/api/site/contact` -> `submit_public_request()` -> should appear as a `requests` row / Approval Task per Sprint 68's existing wiring).
