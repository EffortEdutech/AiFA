...
## 6. What is NOT covered (carried forward, consistent with Sprint 16's own list and this sprint's "Safe to Carry Over")

- **Device registration / DEK bootstrap flow, and wiring `ReadOnlyBanner`/`PrimaryDeviceSettingsCard` into `App.tsx`/`SettingsScreen.tsx`.** This is the same gap Sprint 16 disclosed and it still blocks actual on-device use of everything built this sprint: with no registration screen, `initMobileSync` is never called, no device ever has a `deviceId`, and the new components have no real props to receive at app startup. All of this sprint's logic is unit-tested directly (bypassing the UI layer, same discipline as Sprint 16's write-gate tests) and the components are written against clean, self-contained prop contracts so they can be wired in with no further logic changes once a registration flow exists — but that flow itself is not this sprint's scope (it was never listed in the Sprint 13-20 breakdown at all; worth flagging to the owner as a real planning gap, not just a Sprint 17 leftover).
