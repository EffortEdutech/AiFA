...
-- ------------------------------------------------------------
-- 8b. Ad-hoc (added 5 September 2026, live-debugging fix): sync_envelopes
-- was missing its table-level INSERT grant -- confirmed live this sprint
-- that EVERY web push (the direct-upsert path in
-- packages/core/src/sync/supabaseTransport.ts's pushEnvelope; unlike
-- every other write in this schema, envelope push goes straight through
-- PostgREST rather than a SECURITY DEFINER RPC, so it depends on a plain
-- table grant the way none of this file's other tables do) had been
-- failing with 42501 "permission denied for table sync_envelopes" since
-- the table was first created -- an RLS policy alone was never enough;
-- Postgres also requires the table-level privilege the policy assumes.
-- The failure was invisible end-to-end: pushOutbox's own catch just
-- leaves the envelope queued for the next cycle without surfacing an
-- error, and useWebSync's push call is itself a best-effort .catch(() =>
-- {}). Net effect: sync_envelopes had zero rows for every business,
-- ever -- e.g. NHL Global Solution's Business profile (name/industry)
-- and Notifications had silently never synced past the local device that
-- first set them, appearing as unset defaults on any other device.
-- ------------------------------------------------------------
grant insert on public.sync_envelopes to authenticated;
...
-- ============================================================
-- Sprint 53 (Phase 5, Universal Input Router) — Capture Triage
--
-- Vol 5_5 §7's mandatory "unclassified" fallback: an input the Quick
-- Capture router's domain heuristic could not confidently place in
-- any known domain is stored here instead of being silently dropped
-- or guessed into the wrong domain. Deliberately minimal — a raw-text
-- queue an owner reviews and re-enters manually through the correct
-- per-domain screen; automatic reclassification/bulk-dismiss is
-- Sprint 58's own scope, not this one.
--
-- No domain-specific capability gate on insert/view: an unclassified
-- item could belong to ANY of the eleven Vol 13_1 §3 domains (that's
-- the whole point — the router hasn't determined which one yet), so
-- this is gated on active membership alone, not a false-precision
-- unfitting-domain guess.
--
-- Path A/B note (see Vol_5_5 §3's correction and Sprint 53's own
-- correction note): this table is a genuine Path B (plaintext
-- Supabase) table, deliberately NOT a `business_events` row — Path A
-- is excluded from this router entirely.
-- ============================================================
...
create index if not exists idx_capture_triage_business on public.capture_triage (business_id, created_at desc);
alter table public.capture_triage enable row level security;

create policy "Active members can view their business's capture triage"
  on public.capture_triage for select
  using (exists (
    select 1 from public.business_memberships bm
    where bm.business_id = capture_triage.business_id and bm.user_id = auth.uid() and bm.status = 'active'
  ));

create or replace function public.create_capture_triage_item(
  p_business_id uuid, p_raw_text text, p_detected_domain text default null
) returns public.capture_triage
language plpgsql security definer set search_path = public, auth
as $$
declare
  v_caller_membership_id uuid;
...
grant execute on function public.create_capture_triage_item(uuid, text, text) to authenticated;

create or replace function public.resolve_capture_triage_item(
...
grant execute on function public.resolve_capture_triage_item(uuid, text) to authenticated;

-- Sprint 53 also adds an optional AI-draft-summary passthrough for
-- create_leave_application — the RPC already accepted p_ai_draft_summary
-- (Sprint 35's own signature), nothing called it with a real value until
-- the Quick Capture router. No schema change needed for this half.

-- End of Sprint 53 migration additions.
